...
configs = [
    {"name": "Exp1_baseline",  "kernel_sizes": [3,4,5], "num_filters": 100, "dropout": 0.5, "lr": 0.001},
    {"name": "Exp2_large",     "kernel_sizes": [2,3,4,5], "num_filters": 200, "dropout": 0.5, "lr": 0.001},
    {"name": "Exp3_ablation",  "kernel_sizes": [3],     "num_filters": 100, "dropout": 0.5, "lr": 0.001},
]

results_txt = ""
for cfg in configs:
    print(f"\n{'='*50}\nRunning {cfg['name']}\n{'='*50}")
    model = TextCNN(VOCAB_SIZE, 300, 6, cfg["kernel_sizes"],
                    cfg["num_filters"], cfg["dropout"], embedding_matrix)
    ...
    plt.title(f"TextCNN {cfg['name']} — Training Curve")
    ...
    plt.title(f"TextCNN {cfg['name']} — Confusion Matrix")
    plt.tight_layout()
